...
def show_hist(sample, hist_title):
    ...
    axes.hist(sample, num_of_bins, edgecolor='black', density=True)
...
def show_boxplot(sample, boxpl_title):
    fig, axes = plt.subplots(1,1)
    axes.boxplot(sample, vert=False, whis=[0, 100], patch_artist=False, showmeans=False)
    axes.grid(True, alpha=0.3)
    axes.set_title(boxpl_title)
    plt.show()
...
# генерация массива из значений заданной статистики (stat_func)
...
# поиск несмещённой оценки дисперсии
...
# возвращает функцию плотности вероятности, построенной на несмещённой дисперсии
...
# демонстрация гистограммы бутстрапа среднего арифметического выборки 
def show_bootstr_compar_mean(sample, bootstr_mass, dens_func_teor, num_points):
    ...
    axes.hist(bootstr_mass, num_of_bins, edgecolor='black', density=True)
    axes.plot(x_mass, y_mass_teor,'g-', linewidth=2, label='ЦПТ, теоретическая')
    axes.plot(x_mass, y_mass_samp,'r-', linewidth=2, label='ЦПТ, по выборке')
    axes.plot(x_mass, y_mass_samp_unmoved,'y--', linewidth=2, label='ЦПТ, по выборке,\n с несмещён. дисперс.')
    axes.set_title("Оценка плотности распред. ср. арифм. эл-тов выборки")
    axes.legend()
    plt.show()
...
def show_bootstr_compar_median(sample, bootstr_mass, dens_func_teor, num_points):
    ...
    axes.hist(bootstr_mass, num_of_bins, edgecolor='black', density=True)
    axes.plot(x_mass, y_mass_teor,'y-', linewidth=2, label='теоретическая')
    axes.plot(x_mass, y_mass_samp,'r-', linewidth=2, label='по выборке')
